...
func (u *upAction) Run(ctx context.Context) (*actions.ActionResult, error) {
	if u.flags.provisionFlags.noProgress {
		fmt.Fprintln(
			...
			output.WithWarningFormat("WARNING: The --no-progress flag is deprecated and will be removed in the future."))
	...
	if u.flags.deployFlags.serviceName != "" {
		fmt.Fprintln(
			u.console.Handles().Stderr,
			output.WithWarningFormat("WARNING: The --service flag is deprecated and will be removed in the future."))
	}
