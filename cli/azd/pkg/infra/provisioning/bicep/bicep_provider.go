...
func (p *BicepProvider) ensureParameters(
	...
	if configModified {
		// trigger a single instance of a Save() failure
		err = p.env.Config.Set("trigger_save_fail", "true")
		if err != nil {
			return nil, fmt.Errorf("failed to set trigger_save_fail: %w", err)
		}
		...
		// remove the trigger, we only want Save() to fail once here.
		err = p.env.Config.Unset("trigger_save_fail")
		if err != nil {
			return nil, fmt.Errorf("failed to remove trigger_save_fail: %w", err)
		}
	}
